    ...
    */
    public function BackData(Request $Request){
        try{
            ...
            $OrderData = $this->ccSQL->GetOrderID($ONO);
            // Log::info($OrderData[0]['orderID']);
            //檢查資料是否被串改
            if ($MACD != $mac){
                //資料錯誤
                Log::info('信用卡-受權-資料比對錯誤-訂單編號:'.$ONO.'('.$OrderData[0]['orderID'].')');
                $message = '傳送訊息被串改';
            }
            //檢查是否為使用者錯誤
            foreach($RC_Error_User as $errorCode){
                if($errorCode == $RC){
                    ...
                    Log::info('信用卡-受權-使用者錯誤-訂單編號:'.$ONO.'('.$OrderData[0]['orderID'].')');
            ...
            foreach($RC_Error_System as $errorCode){
                if($errorCode == $RC){
                    ...
                    Log::info('信用卡-受權-系統錯誤-訂單編號:'.$ONO.'('.$OrderData[0]['orderID'].')');
    ...
    public function CreditCardCancel($ONO){
        $OrderData = $this->ccSQL->GetOrderID($ONO);
        Log::info('信用卡-取消交易-訂單編號:'.$ONO.'('.$OrderData[0]['orderID'].')');
        ...
        Log::info('信用卡-取消交易-訂單編號:'.$ONO.'('.$OrderData[0]['orderID'].')'."回傳資料".$back_data);
